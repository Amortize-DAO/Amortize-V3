...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        const block = chain.mineBlock([
            Tx.contractCall('btc-lock', 'lock', [types.principal(beneficiary.address), types.uint(10), types.uint(amount)], deployer.address)
        ]);

        // The lock should be successful.
        block.receipts[0].result.expectOk().expectBool(true);
        // There should be a STX transfer of the amount specified.
        block.receipts[0].events.expectSTXTransferEvent(amount, deployer.address, `${deployer.address}.btc-lock`);
    }
});
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        const block = chain.mineBlock([
            Tx.contractCall('btc-lock', 'lock', [types.principal(beneficiary.address), types.uint(10), types.uint(10)], accountA.address)
        ]);
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        const block = chain.mineBlock([
            Tx.contractCall('btc-lock', 'lock', [types.principal(beneficiary.address), types.uint(targetBlockHeight), types.uint(amount)], deployer.address),
        ]);
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        const block = chain.mineBlock([
            Tx.contractCall('btc-lock', 'claim', [], beneficiary.address),
        ]);

        // The claim was successful and the STX were transferred.
        block.receipts[0].result.expectOk().expectBool(true);
        block.receipts[0].events.expectSTXTransferEvent(amount, `${deployer.address}.btc-lock`, beneficiary.address);
    }
});
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        const block = chain.mineBlock([
            Tx.contractCall('btc-lock', 'claim', [], other.address),
        ]);
